...
void Set_DateTime(uint8_t second, uint8_t minute, uint8_t hour, uint8_t day, uint8_t date, uint8_t month, uint8_t year)
{
		...
		HAL_I2C_Mem_Write_IT(I2CChanel,SLAVE_ADDRESS_DS3231<<1,0,I2C_MEMADD_SIZE_8BIT,send_data,7);
}
...
void Get_DateTime(uint8_t * second, uint8_t * minute, uint8_t * hour, uint8_t * day, uint8_t * date, uint8_t * month, uint8_t * year)
{
	...
	HAL_I2C_Mem_Read(I2CChanel, SLAVE_ADDRESS_DS3231<<1, 0, I2C_MEMADD_SIZE_8BIT, receive_data, 7, 1000);
...
{
	uint8_t temp[2];
	HAL_I2C_Mem_Read(I2CChanel, SLAVE_ADDRESS_DS3231<<1, 0x11, I2C_MEMADD_SIZE_8BIT, temp, 2, 1000);
